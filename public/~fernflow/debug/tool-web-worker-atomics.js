...
((window) => {
  ...
  const getAndSetInstanceId = (instance, instanceId, nodeName) => {
    if (instance) {
      ...
      'number' != typeof (instanceId = instance[InstanceIdKey]) &&
  ...
  const deserializeFromWorker = (
    ...
    if (serializedTransfer) {
      ...
      return 0 === serializedType
        ...
        : 3 === serializedType
        ? getInstance(serializedValue.$winId$, serializedValue.$instanceId$)
        : 5 === serializedType
  ...
  const registerWindow = (worker, $winId$, $window$) => {
    if (!windowIds.has($window$)) {
      ...
      winCtxs[$winId$].$startTime$ = performance.now();
      {
        ...
        logMain(
          `Registered ${winType} window ${normalizedWinId(
            $winId$,
          )} (${$winId$})`,
        );
      }
    ...
    return (worker, msg) => {
      ...
      if (0 === msgType) {
        const initData = (() => {
          ...
          const svg = docImpl.createElementNS(
          ...
          const elms = Object.getOwnPropertyNames(mainWindow)
            ...
            .map((htmlCstrName) => [
              docImpl.createElement(
                getHtmlTagNameFromConstructor(htmlCstrName),
              ),
            ]);
          const elm = elms[0][0];
          const impls = [
