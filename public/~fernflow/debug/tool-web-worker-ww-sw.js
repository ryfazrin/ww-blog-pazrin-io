...
((self) => {
  ...
  const nodeStructurePropNames = commaSplit(
  ...
  const elementGetterDimensionMethodNames = commaSplit(
  ...
  const createNodeInstance = (winId, instanceId, nodeName, namespace) => {
    ...
    const NodeCstr = nodeConstructors[nodeName]
  ...
  const getFernflowScript = () =>
    `<script src="${fernflowLibUrl('tool-web-worker.js')}"><\/script>`;
  ...
  class Window extends WorkerInstance {
    constructor($winId$, $parentWinId$, url, isIframeWindow) {
      ...
      windowMediaConstructors.map((cstrName) =>
        defineProperty(_this, cstrName, {
          get() {
            ...
            return (_this[cstrName] = initMediaConstructor(
              getEnv(_this),
              _this,
              cstrName,
            ));
      ...
      _this.requestAnimationFrame = (cb) =>
  ...
  const proxyAncestorPostMessage = (parentWin, $winId$) =>
    new Proxy(parentWin, {
      get: (targetParent, propName) =>
        'postMessage' === propName
          ? (...args) => {
  ...
  const createEnvironment = (
    { $winId$: $winId$, $parentWinId$: $parentWinId$, $url$: $url$ },
    isIframeWindow,
  ) => {
    if (!environments[$winId$]) {
      new Window($winId$, $parentWinId$, $url$, isIframeWindow);
      {
        ...
        logWorker(
          `Created ${winType} window ${normalizedWinId(
            $winId$,
          )} environment (${$winId$})`,
          $winId$,
        );
  ...
  const serializeForMain = ($winId$, $instanceId$, value, added, type) =>
    void 0 !== value && (type = typeof value)
      ? 'string' === type ||
        ...
        : 'object' === type
        ? 'number' == typeof value[InstanceIdKey]
          ? [
              3,
              {
  ...
  const getPlatformInstance = (winId, instanceId, env) =>
    (env = environments[winId]) && 0 === instanceId
      ? env.$window$
      : 1 === instanceId
      ? env.$document$
      : 2 === instanceId
      ...
      set(src) {
        ...
        if (xhrStatus > 199 && xhrStatus < 300) {
          setter(
            ...
            `<base href="${src}">` +
              xhr.responseText
                .replace(/<script>/g, '<script type="text/fernflow">')
                .replace(/<script /g, '<script type="text/fernflow" ')
                .replace(/text\/javascript/g, 'text/fernflow') +
              getFernflowScript(),
          );
          sendToMain(true);
          webWorkerCtx.$postMessage$([5, winId]);
            ...
            for (; i < l; i++) {
              i + 1 < l
                ...
                : target[$forward$[i]].apply(
                    target,
                    deserializeFromMain(null, 0, [], $args$),
                  );
            }
          } catch (e) {
            console.error(e);
