...
((self) => {
  ...
  const EMPTY_ARRAY = [];
  const defineCstr = (cstrName, Cstr) =>
    (self[cstrName] = defineCstrName(cstrName, Cstr));
      ...
      return new Proxy(instance, {
        get: (target, propName) =>
          'number' == typeof propName
            ? new AudioTrack(winId, instanceId, ['audioTracks', propName])
            : target[propName],
      });
    }
  };
    ...
    buffered: {
      get() {
        if (!this[TimeRangesKey]) {
          this[TimeRangesKey] = new TimeRanges(
            this[WinIdKey],
            this[InstanceIdKey],
            ['buffered'],
          );
